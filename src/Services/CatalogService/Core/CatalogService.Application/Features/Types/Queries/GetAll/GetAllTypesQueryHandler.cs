...
namespace CatalogService.Application.Features.Types.Queries.GetAll
{
    ...
    {
        public async Task<IList<GetAllTypesQueryResponse>> Handle(GetAllTypesQueryRequest request, CancellationToken cancellationToken) => mapper.Map<GetAllTypesQueryResponse, Domain.Entities.Type>(await unitOfWork.GetReadRepository<Domain.Entities.Type>().GetAllAsync(cancellationToken));
    }
}
